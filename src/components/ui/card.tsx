...
export function Card({ title, imageSource }: TaskProps) {
  return (
    <div
      className={`w-48 h-auto bg-[#FAFAFA] border-2 border-[#3B4CCA] rounded-lg  p-6 overflow-hidden flex flex-col items-center`}
    >
      ...
      <Link href={`/vitrine/${title.toLowerCase()}`}>
        <Button label="Detalhes" variant="primary"></Button>
      </Link>
    </div>
  );
}